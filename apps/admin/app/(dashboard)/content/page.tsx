...
function arabicTextProps(value?: string) {
  if (!value || !ARABIC_RE.test(value)) return {};

  return {
    dir: 'rtl',
    lang: 'ar',
    className: 'arabic-content font-quran-core text-right leading-[2.6]',
  };
}
...
export default function ContentReviewPage() {
  ...
  const handlePreview = async (topicId: string) => {
    try {
      setActionLoading(`preview-${topicId}`);
      const response = await fetch(`/api/topics/${topicId}/preview-url`);
      const data = await response.json();

      if (!data.success || !data.data?.url) {
        setResult({ success: false, error: data.error || 'Could not open preview' });
        return;
      }
                ...
                {topics.length === 0 ? (
                  ...
                  topics.map((topic) => (
                    <tr key={topic._id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium text-slate-800" {...arabicTextProps(topic.title)}>
                          {topic.title}
                        </div>
                        <div className="text-xs text-slate-500" {...arabicTextProps(topic.book_id?.subject)}>
                          {topic.book_id?.subject}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-600">
                        <span {...arabicTextProps(topic.chapter_id?.title)}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            onClick={() => handlePreview(topic._id)}
                            disabled={actionLoading === `preview-${topic._id}`}
                            variant="outline"
                            size="sm"
                          >
                            {actionLoading === `preview-${topic._id}` ? '...' : 'Preview'}
                          </Button>
                          <Button
