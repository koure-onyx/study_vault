...
export async function GET(
  ...
  try {
    ...
    const chapterSlug = request.nextUrl.searchParams.get('chapterSlug') || request.nextUrl.searchParams.get('chapterNumber') || '';
    const topicSlug = request.nextUrl.searchParams.get('topicSlug') || '';

    if (!subjectSlug || !chapterSlug || !topicSlug) {
      return NextResponse.json(
        { success: false, error: 'Missing subjectSlug, chapterSlug, or topicSlug' },
        { status: 400 }
      );
    }
    ...
    // Find topic by slug within the chapter
    const topic = await TopicModel.findOne({
      ...
      .populate('book_id', 'title subject slug edition_year seo')
      .populate('program_id', 'name slug')
      .populate('board_id', 'name short_code slug')
      .lean();
